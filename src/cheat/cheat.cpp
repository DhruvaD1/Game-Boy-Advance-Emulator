#include "cheat/cheat.hpp"
#include "memory/bus.hpp"
#include <cstdio>
#include <cstring>
#include <sstream>

static const u32 INIT_V1[4] = {0x09F4FBBD, 0x9681884A, 0x352027E9, 0xF3DEE5A7};
static const u32 INIT_V3[4] = {0x7AA9648F, 0x7FAE6994, 0xC0EFAAD5, 0x42712C57};

static const u8 V1_T1[256] = {
    
};
static const u8 V1_T2[256] = {
    
};
static const u8 V3_T1[256] = {
    
};
static const u8 V3_T2[256] = {
    
};

void CheatEngine::reset_seeds() {
    memcpy(seeds_v1_, INIT_V1, sizeof(seeds_v1_));
    memcpy(seeds_v3_, INIT_V3, sizeof(seeds_v3_));
}

void CheatEngine::tea_decrypt(u32& v0, u32& v1, const u32 key[4]) {
    u32 sum = 0xC6EF3720;
    for (int i = 0; i < 32; i++) {
        v1 -= ((v0 << 4) + key[2]) ^ (v0 + sum) ^ ((v0 >> 5) + key[3]);
        v0 -= ((v1 << 4) + key[0]) ^ (v1 + sum) ^ ((v1 >> 5) + key[1]);
        sum -= 0x9E3779B9;
    }
}

void CheatEngine::reseed_v1(u32* seeds, u16 params) {
    int s0 = params >> 8;
    int s1 = params & 0xFF;
    for (int y = 0; y < 4; y++) {
        for (int x = 0; x < 4; x++) {
            u8 z = V1_T1[(s0 + x) & 0xFF] + V1_T2[(s1 + y) & 0xFF];
            seeds[y] <<= 8;
            seeds[y] |= z;
        }
    }
}

void CheatEngine::reseed_v3(u32* seeds, u16 params) {
    int s0 = params >> 8;
    int s1 = params & 0xFF;
    for (int y = 0; y < 4; y++) {
        for (int x = 0; x < 4; x++) {
            u8 z = V3_T1[(s0 + x) & 0xFF] + V3_T2[(s1 + y) & 0xFF];
            seeds[y] <<= 8;
            seeds[y] |= z;
        }
    }
}

static u32 par3_addr(u32 x) {
    return (x & 0xFFFFF) | ((x << 4) & 0x0F000000);
}

void CheatEngine::decode_entry_v1(CheatEntry& entry) {
    entry.decoded.clear();
    for (auto& r : entry.raw_codes) {
        u32 v0 = r.first, v1 = r.second;
        tea_decrypt(v0, v1, seeds_v1_);

        if (v0 == 0xDEADFACE) {
            reseed_v1(seeds_v1_, (u16)(v1 & 0xFFFF));
            continue;
        }

        u8 type = (v0 >> 28) & 0xF;
        if (type == 0xF) continue;

        DecodedCode dc;
        dc.type = type;
        dc.addr = v0 & 0x0FFFFFFF;
        dc.value = v1;
        entry.decoded.push_back(dc);
    }
}

void CheatEngine::decode_entry_v3(CheatEntry& entry) {
    entry.decoded.clear();
    for (size_t i = 0; i < entry.raw_codes.size(); i++) {
        u32 op1 = entry.raw_codes[i].first;
        u32 op2 = entry.raw_codes[i].second;
        tea_decrypt(op1, op2, seeds_v3_);

        if (op1 == 0xDEADFACE) {
            reseed_v3(seeds_v3_, (u16)(op2 & 0xFFFF));
            continue;
        }
        if (op2 == 0x001DC0DE) continue;
        if ((op1 >> 24) == 0xC4) continue;
        if (op1 == 0 && op2 == 0) continue;

        if (op1 == 0) {
            u8 patch_type = (op2 >> 24) & 0xFE;
            if (patch_type == 0x18 || patch_type == 0x1A || patch_type == 0x1C || patch_type == 0x1E) {
                u32 rom_addr = (op2 & 0x00FFFFFF) << 1;
                if (i + 1 < entry.raw_codes.size()) {
                    u32 p1 = entry.raw_codes[i + 1].first;
                    u32 p2 = entry.raw_codes[i + 1].second;
                    tea_decrypt(p1, p2, seeds_v3_);
                    i++;

                    DecodedCode dc;
                    dc.type = TYPE_ROM_PATCH;
                    dc.addr = rom_addr;
                    dc.value = p1 & 0xFFFF;
                    entry.decoded.push_back(dc);
                }
            }
            continue;
        }

        u32 siz = (op1 >> 25) & 3;
        u32 addr = par3_addr(op1);

        DecodedCode dc;
        dc.addr = addr;
        dc.value = op2;

        if (siz == 0) {
            dc.type = TYPE_WRITE8;
            dc.value = op2 & 0xFF;
        } else if (siz == 1) {
            dc.type = TYPE_WRITE16;
            dc.value = op2 & 0xFFFF;
        } else {
            dc.type = TYPE_WRITE32;
        }

        entry.decoded.push_back(dc);
    }
}

void CheatEngine::decode_entry(CheatEntry& entry) {
    CheatEntry test_v1 = entry;
    u32 saved_v1[4], saved_v3[4];
    memcpy(saved_v1, seeds_v1_, sizeof(saved_v1));
    memcpy(saved_v3, seeds_v3_, sizeof(saved_v3));

    decode_entry_v1(test_v1);

    bool v1_valid = !test_v1.decoded.empty();
    for (auto& dc : test_v1.decoded) {
        u8 t = dc.type;
        if (t != 0 && t != 1 && t != 2 && t != 3 && t != 6 && t != 8 && t != 0xD && t != 0xE) {
            v1_valid = false;
            break;
        }
    }

    if (v1_valid) {
        entry.decoded = test_v1.decoded;
        memcpy(seeds_v3_, saved_v3, sizeof(seeds_v3_));
        return;
    }

    memcpy(seeds_v1_, saved_v1, sizeof(seeds_v1_));
    decode_entry_v3(entry);
}

bool CheatEngine::parse_code_line(const std::string& line, u32& first, u32& second) {
    if (line.size() < 17) return false;
    char* end1;
    first = (u32)strtoul(line.c_str(), &end1, 16);
    if (end1 == line.c_str()) return false;
    while (*end1 == ' ') end1++;
    char* end2;
    second = (u32)strtoul(end1, &end2, 16);
    if (end2 == end1) return false;
    return true;
}

bool CheatEngine::load(const std::string& path) {
    cheats_.clear();
    dirty_ = false;
    reset_seeds();

    FILE* f = fopen(path.c_str(), "r");
    if (!f) return false;

    char line[512];
    CheatEntry current;
    bool has_entry = false;

    while (fgets(line, sizeof(line), f)) {
        char* nl = strchr(line, '\n');
        if (nl) *nl = '\0';
        char* cr = strchr(line, '\r');
        if (cr) *cr = '\0';

        std::string s = line;
        if (s.empty()) continue;

        if (s[0] == '[') {
            if (has_entry && !current.raw_codes.empty()) {
                decode_entry(current);
                cheats_.push_back(current);
            }

            current = CheatEntry();
            has_entry = true;

            size_t close = s.find(']');
            if (close != std::string::npos) {
                current.name = s.substr(1, close - 1);
                std::string rest = s.substr(close + 1);
                while (!rest.empty() && rest[0] == ' ') rest = rest.substr(1);
                current.enabled = (rest != "OFF");
            } else {
                current.name = s.substr(1);
                current.enabled = true;
            }
            continue;
        }

        if (has_entry) {
            u32 first, second;
            if (parse_code_line(s, first, second)) {
                current.raw_codes.push_back({first, second});
            }
        }
    }

    if (has_entry && !current.raw_codes.empty()) {
        decode_entry(current);
        cheats_.push_back(current);
    }

    fclose(f);

    return true;
}

bool CheatEngine::save(const std::string& path) {
    FILE* f = fopen(path.c_str(), "w");
    if (!f) return false;

    for (auto& cheat : cheats_) {
        fprintf(f, "[%s]", cheat.name.c_str());
        if (!cheat.enabled) fprintf(f, " OFF");
        fprintf(f, "\n");
        for (auto& code : cheat.raw_codes) {
            fprintf(f, "%08X %08X\n", code.first, code.second);
        }
        fprintf(f, "\n");
    }

    fclose(f);
    dirty_ = false;
    return true;
}

void CheatEngine::apply() {
    if (!bus_) return;

    for (auto& cheat : cheats_) {
        if (!cheat.enabled) continue;

        for (size_t i = 0; i < cheat.decoded.size(); i++) {
            auto& dc = cheat.decoded[i];

            switch (dc.type) {
                case TYPE_WRITE8:
                    bus_->write8(dc.addr, (u8)(dc.value & 0xFF));
                    break;
                case TYPE_WRITE16:
                    bus_->write16(dc.addr, (u16)(dc.value & 0xFFFF));
                    break;
                case TYPE_WRITE32:
                case 3:
                    bus_->write32(dc.addr, dc.value);
                    break;
                case TYPE_ROM_PATCH: {
                    u32 offset = dc.addr;
                    u16 val = (u16)(dc.value & 0xFFFF);
                    if (offset + 1 < bus_->rom_size) {
                        bus_->rom[offset] = val & 0xFF;
                        bus_->rom[offset + 1] = (val >> 8) & 0xFF;
                    }
                    break;
                }
                case TYPE_COND_EQ:
                    if (i + 1 < cheat.decoded.size()) {
                        u16 mem_val = bus_->read16(dc.addr);
                        if (mem_val != (u16)(dc.value & 0xFFFF)) {
                            i++;
                        }
                    }
                    break;
            }
        }
    }
}

void CheatEngine::toggle(int index) {
    if (index >= 0 && index < (int)cheats_.size()) {
        cheats_[index].enabled = !cheats_[index].enabled;
        dirty_ = true;
    }
}

void CheatEngine::remove(int index) {
    if (index >= 0 && index < (int)cheats_.size()) {
        cheats_.erase(cheats_.begin() + index);
        dirty_ = true;
    }
}

bool CheatEngine::add(const std::string& name, const std::string& code_text) {
    CheatEntry entry;
    entry.name = name;
    entry.enabled = true;

    std::istringstream stream(code_text);
    std::string line;
    while (std::getline(stream, line)) {
        while (!line.empty() && (line.back() == '\r' || line.back() == ' ')) line.pop_back();
        if (line.empty()) continue;
        u32 first, second;
        if (parse_code_line(line, first, second)) {
            entry.raw_codes.push_back({first, second});
        }
    }

    if (entry.raw_codes.empty()) return false;
    decode_entry(entry);
    cheats_.push_back(entry);
    dirty_ = true;
    return true;
}
